...
EXTRA = {
'SOP-001': dict(trigger='Gebruik deze SOP bij een nieuwe werkwijze, een fout met herhalingsrisico, een wijziging in wet- of klantvereisten, een nieuwe logistieke partner, tijdens de wekelijkse sturing en tijdens de kwartaalreview.', decisions=[('Direct herstellen','Kleine, eenmalige fout met beperkte impact en duidelijke oorzaak.'),('Verbeteractie','Fout kan terugkomen of raakt meerdere orders, producten of partners.'),('Stop/extern advies','Mogelijk productveiligheids-, wettelijk of ernstig financieel risico.')], controls=['Geen actie zonder eigenaar en datum.','Een kritieke actie wordt door de andere compagnon beoordeeld.','Een KPI wordt alleen gebruikt met een vaste definitie, bron en vervolgstap.','Een SOP wordt alleen uitgebreid wanneer de extra stap werkelijk uitvoerbaar is.'], outputs=['Actuele werkwijze','Gesloten actie met bewijs','Wekelijks stuurbeeld','Kort kwartaalreviewverslag'], example='Een order die zonder voortgang blijft staan, wordt niet alleen als cijfer getoond. In de weeksturing krijgt de blokkade een eigenaar en concrete vervolgstap.'),
'SOP-002': dict(trigger='Gebruik vóór eerste inkoop/verkoop van een artikel en bij wijziging van leverancier, verpakking, markt, certificaat, batch-/lotvereiste of houdbaarheid.', decisions=[('Vrijgeven','Verplichte gegevens en documenten zijn compleet en passend.'),('Voorwaardelijk','Niet-kritieke informatie ontbreekt; beperking en einddatum zijn vastgelegd.'),('Blokkeren','Kritisch document, identiteit, productkoppeling of markttoegang is onzeker.')], controls=['Minimaal zijn HS-code, oorsprong, gewicht, fabrikantreferentie, productomschrijving, prijs en verpakkingsgrootte bekend.','SKU, eenheid, HS-code, oorsprong en documentstatus worden door de andere compagnon gecontroleerd.','Vervaldatum en product-/leverancierskoppeling staan bij elk kritisch document.'], outputs=['Vrijgegeven of geblokkeerd productdossier','Correcte artikelgegevens in de gebruikte administratie'], example='Een aantrekkelijke partij medische producten wordt niet aangeboden op basis van alleen een prijslijst. Eerst worden de bevestigde minimumvelden en de vereiste markt-/productdocumenten compleet gemaakt.'),
'SOP-003': dict(trigger='Gebruik bij een nieuwe leverancier, een gewijzigde betaalrekening, een kritieke dienstverlener, een grote nieuwe handelsmogelijkheid of terugkerende prestatieproblemen.', decisions=[('Voorkeur fabrikant','Rechtstreeks bij de fabrikant, bij voorkeur met 30 dagen betaling en franco levering.'),('Medische groothandel','Alleen wanneer de website merken toont die Inco-Source al inkoopt en de overige verificaties slagen.'),('Kritisch','Hoge klantimpact, moeilijk vervangbaar of grote product-/financiële afhankelijkheid.'),('Blokkeren','Fraude-indicatie, onbeheerst productrisico of ontbrekende kritieke documentatie.')], controls=['Leverancierstype, betalingstermijn en leverconditie vastleggen.','Bij een groothandel het website-/merkenbewijs bewaren.','Bankwijzigingen altijd via onafhankelijk contactkanaal bevestigen.','De eerste levering van een nieuwe leverancier extra controleren.'], outputs=['Leveranciersstatus en voorwaarden','Bewijs van verificatie','Acties bij afwijkingen'], example='Een scherpe aanbieding van een algemene groothandel is niet voldoende. De voorkeursroute is de fabrikant; een alternatief moet aantoonbaar een medische groothandel zijn en aan het merkencriterium voldoen.'),
...
'SOP-009': dict(trigger='Gebruik voor iedere klantorder, inclusief kansgestuurde verkoop, herhaalorder, DAP-uitzondering, spoedorder en sample op klantverzoek.', decisions=[('Vrijgeven','EXW minimaal € 5.000, of DAP als vastgelegde uitzondering vanaf € 10.000, én minimaal 9% nettomarge en € 500 nettowinst; overige ordercontroles zijn akkoord.'),('Hold','Kritieke informatie, voorraadzekerheid, financiële grens of akkoord op de uitzondering ontbreekt.'),('Afwijzen/heronderhandelen','Minimumwaarde, marge, winst, risico, betaalconditie of uitvoering is niet verantwoord.')], controls=['Order en facturatie worden in Odoo vastgelegd.','Controleer zowel nettomarge als nettowinst; één van beide missen betekent geen vrijgave.','DAP en overige Incoterm-uitzonderingen door de andere compagnon laten controleren.','Samples alleen op klantverzoek en waar mogelijk via de leverancier laten meelopen.'], outputs=['Bevestigde klantorder in Odoo','Duidelijke uitvoeringsinstructie','Backorder- of holdstatus'], example='Een EXW-order van € 6.000 met 10% nettomarge maar slechts € 400 nettowinst wordt niet vrijgegeven: beide financiële grenzen moeten worden gehaald.'),
