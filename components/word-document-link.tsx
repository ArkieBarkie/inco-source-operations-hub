...
function documentUrl(fileName:string){
  ...
  return `${window.location.origin}${basePath}/sop-documents/${encodeURIComponent(fileName)}`;
}
...
export function WordDocumentLink({fileName,compact=false,showDownload=false}:{fileName:string;compact?:boolean;showDownload?:boolean}){
  ...
  const openInWord=()=>{window.location.href=`ms-word:ofe|u|${url||documentUrl(fileName)}`};
  return <div className={`no-print flex ${compact?'flex-wrap items-center gap-2':'flex-col gap-2'}`}>
    <button type="button" onClick={openInWord} className={`inline-flex items-center justify-center gap-2 rounded-xl bg-blue-50 font-bold text-accent transition hover:bg-blue-100 ${compact?'px-3 py-2 text-xs':'w-full px-4 py-3 text-sm'}`}>
