  ...
  return (
    <div className="grid min-h-[680px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-soft xl:grid-cols-[310px_1fr]">
      <aside className="border-b bg-navy p-6 text-white xl:border-b-0 xl:border-r xl:border-white/10">
        ...
        <p className="mt-5 text-sm leading-6 text-blue-100">Assistent over zendingen, relaties, planning, acties, orderchecks, voorraad, SOP’s en intern versus 3PL. Kan nu ook opvolgacties en afwijkingen veilig voorbereiden.</p>
        ...
        <ApiStatusCard status={apiStatus} checking={checkingApi} onTest={() => void testApiConnection()} canTest={canAdmin} />
...
function ApiStatusCard({status, checking, onTest, canTest}: {status: ApiStatus | null; checking: boolean; onTest: () => void; canTest: boolean}) {
  if (!status) {
  ...
  if (!status.configured || status.connection === 'missing_key') {
    return <div className="mt-4 rounded-2xl border border-amber-300/40 bg-amber-300/10 p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {canTest && <button onClick={onTest} disabled={checking} className="rounded-lg border border-white/20 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">{checking ? 'Controleren…' : 'Opnieuw controleren'}</button>}
      </div>
    </div>;
  }
  ...
  return <div className={`mt-4 rounded-2xl border p-4 ${ok ? 'border-emerald-300/30 bg-emerald-300/10' : status.connection === 'error' ? 'border-red-300/30 bg-red-300/10' : 'border-white/10 bg-white/5'}`}>
    ...
    {canTest && <button onClick={onTest} disabled={checking} className="mt-3 rounded-lg border border-white/20 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">{checking ? 'Verbinding testen…' : ok ? 'Opnieuw testen' : 'Test verbinding'}</button>}
  </div>;
}
