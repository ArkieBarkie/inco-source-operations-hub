  ...
  return (
    <div className="container-page">
      ...
      <section className="mt-7"><SectionTitle title="Kern-KPI’s" note="Groen haalt het doel, amber vraagt aandacht en rood vraagt directe bijsturing." /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <KpiCard label="Orderbesluit definitief" value={`${kpis.orders.decisionRate}%`} note={`${kpis.orders.open} ordervrijgaven nog open`} target="90%" status={statusForTarget(kpis.orders.decisionRate, 90)} />
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
        ...
        <section className="card p-6"><SectionTitle title="Financieel beslisrisico" note="Alleen ordervrijgaven zonder definitief besluit." /><div className="mt-5 rounded-2xl bg-red-50 p-5"><p className="text-xs font-bold uppercase tracking-wide text-red-700">Orderwaarde open</p><div className="mt-2 text-3xl font-bold text-red-800">{euro(kpis.orders.orderValueAtRisk)}</div><p className="mt-1 text-sm text-red-700">{euro(kpis.orders.netProfitAtRisk)} nettowinst gekoppeld</p></div><div className="mt-4 grid grid-cols-2 gap-3"><KpiCard label="Vrijgegeven" value={kpis.orders.released} note={`${kpis.orders.releaseRate}% van alle controles`} status="good" /><KpiCard label="Nog open" value={kpis.orders.open} note="Hold of escalatie" status={kpis.orders.open ? 'risk' : 'good'} /></div><Link href="/ordercheck" className="mt-4 inline-flex text-sm font-bold text-accent">Open ordervrijgave →</Link></section>
      </div>

      <details className="card mt-7 p-6"><summary className="cursor-pointer list-none font-bold text-navy">Operationele verdieping <span className="ml-2 text-sm font-normal text-slate-500">Cijfers en routes per procesgebied</span></summary><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <details className="card mt-4 p-6"><summary className="cursor-pointer list-none font-bold text-navy">KPI-definities <span className="ml-2 text-sm font-normal text-slate-500">Open voor rekenregels en uitleg</span></summary><div className="mt-5 grid gap-4 text-sm md:grid-cols-2 xl:grid-cols-4"><Definition title="Op tijd geleverd">Werkelijke levering ligt op of vóór de geplande levering, alleen voor dossiers met beide tijdstippen.</Definition><Definition title="Direct afrondbaar">Fysieke bevestiging, aantallen en documenten zijn aanwezig en er staat geen blokkade open.</Definition><Definition title="Externe bevestiging open">Voor de externe stroom is een ontvangst- of uitleverbevestiging vereist.</Definition><Definition title="Operationele score">Gewogen combinatie van leverbetrouwbaarheid, planning, acties, ordervrijgave en voorraadbeschikbaarheid.</Definition><Definition title="Bloktijd bevestigd">Niet-geannuleerde planning met schriftelijk bevestigde bloktijd.</Definition><Definition title="Acties onder controle">Open acties waarvan de deadline vandaag of later ligt.</Definition><Definition title="Orderbesluit definitief">Ordervrijgaven met uitkomst Vrijgegeven of Afgewezen.</Definition><Definition title="Voorraad beschikbaar">Beschikbaar aantal gedeeld door de totale geregistreerde voorraad.</Definition></div></details>
    </div>
  );
}
